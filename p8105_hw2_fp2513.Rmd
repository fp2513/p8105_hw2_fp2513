...
I decided to full join between the Bakers_Bakes_df and Results_df, because I noticed that when I left joined the two, all bakers were there but there were some missing observations of results. This was spotted through `anti_join` to see if there were any results in the Results_df but not in the resultant Bakers_Bakes_Results_Join_df. Furthermore, I noticed that there were only 573 observations in the resultant data frame but 1137 observations in the Results_df which meant that some results were missing. I realised that the results included NA when the baker had been outed, whereas in the Bakers_Bakes_df this only included information of when the bakes and therefore when the baker was still 'in' the show. To solve this, I decided to `full_join` the Bakers_Bakes_df and Results_df instead. This way I would maintain consistent information on the baker even when they had left the show.(cleaning up the resulting dataframe also involved deleting the baker column that I recreated for the Bakers_df of only the first name and retaining the first and last name 'baker_name' column). My only concern was that there are 
...
Lastly, I arranged the final Bakers_Bakes_Results_df, so that it is ordered by series, episode and baker's name. 
