...
Matching datasets by baker name First case match so that there are last
names as well? since
...
A left join means that all rows from Bakers_df will be preserved, and
matching rows from Bakes_Count_df will be added where they exist.

by = c(“baker” = “baker”, “series” = “series”): The argument specifies
which columns to join on. In this case, the baker column from Bakers_df
is matched with the baker column from Bakes_Count_df, and similarly for
the series column. The resulting data frame, Bakers_Bakes_Join_df,
includes all columns from Bakers_df. If a baker in Bakers_df corresponds
to multiple entries in Bakes_Count_df, that baker will appear in
multiple rows, each with the respective bake.

When performing a join in R using left_join, if there are multiple
matches for the keys specified in the by argument, the resulting data
frame will include multiple rows for each match.

I wanted that each row in the Bakes dataframe would join onto where the
baker and series matched
